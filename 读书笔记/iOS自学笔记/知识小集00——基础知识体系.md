...
* **UIView绘制流程**：
  ...
  * CALayer的后备存储Backing Store : 当我们调用CALayer的drawInContext方法时，layer会自动创建一块以layer大小成正比的内存区域，这个区域就是后备存储（backing store）。所以也最好不要随意重写drawRect:等相关方法。
  ...
  [最详细的UI绘制原理](https://juejin.cn/post/7081659568920461348)
...
struct __CFRunLoop {
    ...
    __CFPort weakUpPort;
    CFMutableSetRef _commonModes; // 存放common Modes
    CFMutableSetRef _commonModeItems; // 存放common Modes下的所有Source/Timer/Observer
...
[runloop详解2](https://blog.csdn.net/cheng_lin0201/article/details/138752819)
